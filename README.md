...
## 2. 网站定位（一句话）
“一个面向新粉的入坑导航 + 面向粉丝的每周信息枢纽：日程表、直播记录、数据成长、精选二创/切片/回放推荐。”

## 3. 目标用户
- 新粉（核心）：想快速了解两位是谁、从哪里开始看
- 老粉：想每周看日程表、快速找回放/切片、看成长数据
- 路人：被海报吸引，想知道“这是啥企划、值得关注吗”

## 4. 核心目标（North Star）
让用户在 30 秒内完成：
- 明白两位是谁 + 找到本周直播日程表 + 看到“推荐从这里开始看”的内容入口
...
## 6. 信息架构（Sitemap）
- `/` 首页（氛围 + 快入口 + 本周日程表预览 + 最新推荐）
- `/start` 入坑导航（新粉路线 + 成员入口 + 必看推荐）
- `/members/fiona` 心宜
- `/members/gladys` 思诺
- `/schedule` 直播日程表（本周/下周）
- `/streams` 直播记录列表
...
## 12. 内容维护流程（每周 10 分钟版）
每周固定流程：
1) 更新 `schedule`：填本周/下周日程表（没有链接先留空）
...
### 12.1 从日程图自动提取（Windows）
- 脚本命令：`npm run extract:schedule:image -- <图片路径> <年份>`
- 示例：`npm run extract:schedule:image -- C:\calendar\2026-02-09.jpg 2026`
- 可选参数（按位置）：`[WeekStart] [Out]`
  - 示例：`npm run extract:schedule:image -- C:\calendar\2026-02-09.jpg 2026 2026-02-09 src/content/schedule/2026-02-09.md`
- 输出：`src/content/schedule/<weekStart>.md`
- 调试信息：`scripts/_tmp/ocr-debug-*.json`
- 说明：该脚本使用 Windows 本地 OCR（`zh-Hans-CN`），会自动提取心宜/思诺的时间、成员和标题；结果是草稿，请人工复核后发布。

### 12.2 直接抓置顶动态图片（B站）
- 拉取置顶动态图片：`npm run fetch:schedule:image -- 3493085336046382`
- 结果图片默认保存到：`scripts/_tmp/schedule-images/<mid>/`
- 一步完成“抓图 + OCR提取 + 生成 schedule 文件”：
  - `npm run update:schedule:from-pinned -- 3493085336046382 2026`
- 可选参数（按位置）：`[Mid] [Year] [WeekStart] [Out]`
- 首次先设置 Cookie（推荐自动提取）：
  - 复制动态接口 cURL 后执行：`npm run set:bili-cookie`
  - 会保存到仓库根目录 `.bili-cookie.txt`（已加入 `.gitignore`）
- 也可手动临时设置：`$env:BILI_COOKIE='SESSDATA=...; bili_jct=...;'`
- 说明：脚本会优先找“置顶 + 有图片”的动态；如果网络/API返回异常，会直接报错退出。
